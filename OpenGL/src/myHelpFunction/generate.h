...
std::vector<float> generateSphereVertices(float x, float y, float z, float radius, int numSlices, int numStacks)
{
    ...
    for (int i = 0; i <= numStacks; i++) {
        ...
        for (int j = 0; j <= numSlices; j++) {
            ...
            vertices.emplace_back(radius * sin(phi) * cos(theta) + x);
            vertices.emplace_back(radius * sin(phi) * sin(theta) + y);
            vertices.emplace_back(radius * cos(phi) + z);
        }
    }
...
std::vector<unsigned int> generateSphereIndices(int numSlices, int numStacks)
{
...
void generateSphereVertices(float radius, int numSlices, int numStacks,
    std::vector<glm::vec3>& vertices, std::vector<float>& scalarData)
{
    vertices.clear();
    scalarData.clear();
    vertices.reserve((numStacks + 10) * (numSlices + 10) );
    scalarData.reserve((numStacks + 10) * (numSlices + 10) );
    float phi, theta;
    glm::vec3 vertex;
    float scalar;
    for (int i = 0; i <= numStacks; i++) {
    ...
    std::vector<double>& vertices, std::vector<float>& normals, std::vector<float>& scalarData)
{
    ...
    normals.clear();
    scalarData.clear();
    vertices.reserve(20 * (numStacks + 1) * (numSlices + 1));
    normals.reserve(20 * (numStacks + 1) * (numSlices + 1));
    scalarData.reserve(20 * (numStacks + 1) * (numSlices + 1));
...
{
    indices.clear();
    indices.reserve(200 * numStacks * numSlices * 6);
